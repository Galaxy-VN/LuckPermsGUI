...
object AddPermissions {
    ...
    fun displayFor(player: Player, user: User) {
        player.openMenu<Linked<Permission>>("Add Permissions") {
            rows(6)
            slots(
                listOf(
            ...
            elements {
                plugin.server.pluginManager.permissions.sortedBy { it.name }
            }
            ...
            set(52, buildItem(XMaterial.COMPASS) {
                ...
                player.sendLang("Player-Add-Permission")
                player.nextChat {
                    user.data().add(Node.builder(Coerce.toString(it)).build())
            ...
            onClick { event, element ->
                if (event.clickEvent().isRightClick) {
                    player.closeInventory()
                    player.sendLang("Player-Time-Permission")
                    player.sendMessage("§eExample: 1d 15m")
                    player.nextChat {
                        user.data().add(Node.builder(element.name).expiry(Time.parseDuration(it.replace(" ", ""))).build())
                        LuckPermsProvider.get().userManager.saveUser(user)
                        player.sendLang("Plugin-Success")
                    }
                }
                if (event.clickEvent().isLeftClick) {
                    player.closeInventory()
                    user.data().add(Node.builder(element.name).build())
                    LuckPermsProvider.get().userManager.saveUser(user)
                    player.sendLang("Plugin-Success")
                }
            }
